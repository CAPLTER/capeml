...
A template work flow to generating a complete EML record is included
with this package. The template is automatically generated if a new
project is created with `write_directory`, which also generates a
`config.yaml` file and new directory, or can be copied from the template
...
the current (default) or specified path. The package scope and dataset
number (e.g., knb-lter-cap.652) is passed as an argument, with the
package identifier (sans the version number) becoming the directory
name. Within the newly created directory, a template work flow with the
project directory name is generated and a `config.yaml`. In
`config.yaml`, the dataset number and dataset package identifier are
generated as parameters. New projects assume a version number = 1, from
which the package identifier (as `packageIdent`) is generated in
`config.yaml`.

#### existing projects

For existing projects, if `config.yaml` does not already exist,
`write_config` will generate `config.yaml` with the package identifier
and dataset number (e.g., knb-lter-cap.652) passed as an argument to the
function. A version number (default = 1) can be passed as a separate
argument.
...
The dataset number (e.g., 520) is read from the `packageNum` parameter
of `config.yaml`. The package identifier (e.g. knb-lter-cap.521.4) is
read from the `packageIdent` parameter of `config.yaml`.
...
must be a markdown file). Alternatively, the work flow below is an
enhanced approach of developing methods if provenance data are required
or there are multiple methods files. If `enhancedMethods` exists, it
will override arguments passed to `methodsFile` in `create_dataset`.
