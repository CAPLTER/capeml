...
and this object name is referenced in the EML metadata. Note that the
project-id is not passed to any of the functions, and must exist in
...
utility that can convert other citation formats, such as .ris, to bibtex. can
convert ris to an intermediate xml format and then convert that to bibtex.

```{sh usage citation, eval=FALSE}
...
citaions as in the example below where we added the citation for the Tellman
dissertation to 
