...
- `write_config` generates `config.yaml` with the package scope and identifier
(e.g., "edi", 521) passed as an argument to the function. A version number
(default = 1) can be passed as a separate argument.
- `write_template` generates a template work flow as a Quarto (qmd) file named
with the package scope and identifier.
- `write_people_template` generates a template yaml file for providing metadata
regarding project personnel.
