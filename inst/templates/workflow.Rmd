...
See the `gioseml` package provides a convenience function (`gioseml::create_role`) for generating CAP LTER investigators from an ASU Global Institute of Sustainability and Innovation database. The first example in the `people` chunk (Sean Payton) demonstrates how to generate an EML type `creator` without use of the `gioseml` tool suite - simply change `creator` to the desired role type (e.g., `metadataProvider`, `associatedParty`).
